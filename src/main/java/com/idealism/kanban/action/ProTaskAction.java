...
@Controller("ProTaskAction")
public class ProTaskAction extends AjaxActionBase implements ModelDriven<ProTaskInfo> {
	...
	public String View() {
		...
		if (id > 0 && user_id>0) {
			...
			Boolean canOperation = proTaskService.CheckCanOperation(id, user_id);
			...
			if(target_id>0 && user_id>0){
				...
				if(proTaskInfo.getReceive_task_user()>0){
					...
					if(receive_task_user_id>0){
						...
						proTask.setStart_time(new Date(System.currentTimeMillis()));
