...
<body style="width:100%;height:100%">
	...
	<div>
		<h1>
		<s:if test="#request.Info.request_id>0"><s:text name="help.edit" /></s:if>
		<s:else><s:text name="help.add" /></s:else>
		</h1>
	</div>
	<div>
		...
		$.ajax({
			...
			success:function SubmitOK(data){
				...
				if (json_data[0].error != "") {
					...
					window.parent.window.location.href = "HelpView?id="+json_data[0].content;
				}
			}
		});
	}
	...
	function AdvancedSearch(){
		window.open('UserSearch?usersSelectType=radio&callBackFunc=searchUsers');
	}
