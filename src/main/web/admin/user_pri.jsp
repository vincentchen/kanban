...
<body style="width:100%;height:100%;">
	...
	<div class="boxradius" style="margin:0px auto;width:98%;text-align:left;">
		<input id="id" type="hidden" value="<s:property value="#Global.RegisterID('UserID',#request.User.user_id)" />">
		<div style="width:100%;">
			<table class="gTab">
				<tr style="border-bottom:1px solid #E5E5E5;">
					...
					<td colspan="3" style="text-indent: 2em;">
						<h1>
							<s:property value="#request.User.user_name" />(<s:property value="#request.User.user_account" />)
						</h1></td>
				</tr>
				<tr>
					<td colspan='4'>
						...
						<table class="gListTab">
							...
							<s:iterator value="#request.Pri" status="st">
								<tr style="border-bottom:1px dashed #E5E5E5">
									...
									<td>
										<input name="PriAdd" type="checkbox" <s:if test="#request.Pri[#st.index][4].ADD>0">checked</s:if>>
									</td>
									<td>
										<input name="PriDel" type="checkbox" <s:if test="#request.Pri[#st.index][4].DEL>0">checked</s:if>>
									</td>
									<td>
										<input name="PriEdit" type="checkbox" <s:if test="#request.Pri[#st.index][4].WRITE>0">checked</s:if>>
									</td>
									<td>
										<input name="PriRead" type="checkbox" <s:if test="#request.Pri[#st.index][4].READ>0">checked</s:if>>
									</td>
								</tr>
							</s:iterator>
							<tr>
	...
	<script type="text/javascript">
		...
		function Submit(){
			...
			$.ajax({
				type:"POST",
				url:"<%=basePath%>admin/AdminUserPriSave",
				data:{
					"id":$("#id").val(),
					"privilege":priStr,
				},
