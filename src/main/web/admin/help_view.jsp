...
<body style="width:100%;height:98%;">
	...
	<div class="boxradius" style="margin:0px auto;width:98%;height:100%;text-align:left;">
		...
		<div style="width:100%;border-bottom:1px solid #E5E5E5;">
			<table class="gTab">
				<tr>
					<td style="width:30px;"><a href="javascript:void(0);" onclick="javascript:window.location.href='<%=basePath%>admin/AdminHelpList?search=1';"><img alt="" src="<%=basePath%>image/left.png">
					</a></td>
					...
					<s:if test="#session.IdeaVerifyPri.WRITE>0">
						<td style="width:80px;"><a href="javascript:void(0);" onclick="Edit();"><img src="<%=basePath%>image/edit.png"><s:text name="edit" /></a>
						</td>
					</s:if>
					...
					<s:if test="#request.Help.help_user_id<=0 && #session.IdeaVerifyPri.DEL>0">
						<td style="width:80px;">
	...
	function OpenDownload(){
		CreatePopDiv("DownLoad","70%","70%","<%=basePath%>admin/AdminHelpDownload?id="+$("#HelpID").val());
	}
	...
	<s:if test="#session.IdeaVerifyPri.WRITE>0">
	function Edit(){
		CreatePopDiv("AddHelp","70%","70%","<%=basePath%>admin/AdminHelpEdit?id="+$("#HelpID").val());
	}
	</s:if>
		...
		$.ajax({
			...
			success:function(data){
				if (data != "ok") {
					...
					window.location.href="<%=basePath%>admin/AdminHelpList?search=1";
				}
			}
		});
	}
