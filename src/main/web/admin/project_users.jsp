...
<body style="width:100%;height:100%;overflow:hidden">
	...
	<input id="proID" type="hidden" value="<s:property value='#request.ProInfo.id' />">
		...
		<table class="gTab" >
			<tr style="height:43px;">
				...
				<td style="float:right;width:100px">
					...
					<h2 style="height:43px;line-height:43px;"><a href="javascript:void(0);" onclick="AdvancedSearch();"><img src="<%=basePath%>image/add_small.png" ><s:text name="add"/></a></h2>
					</s:if>
				</td>
			</tr>
		</table>
	...
	<script type="text/javascript">
		...
		function ForcedReceive(proID){
			if($("#user_id").val() == ""){
				...
				$.ajax({
					type : "POST",
					url : "<%=basePath%>admin/AdminProjectAddRelUsers",
					...
					success : function ForcedReceiveOK(data) {
						...
						if (json_data[0].error != "") {
							...
							window.location.href = "<%=basePath%>admin/AdminProjectUsers?id="+proID;
						}
					}
				});
			}
		...
		function DeleteRelUser(userID){
			...
			$.ajax({
				type : "POST",
				url : "<%=basePath%>admin/AdminProjectDeleteRelUsers",
		...
		function AdvancedSearch(){
			window.open('<%=basePath%>UserSearch?usersSelectType=checkbox&callBackFunc=saveUsers');
		}
