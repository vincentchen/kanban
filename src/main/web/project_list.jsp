...
<body style="width:100%;height:100%;">
				...
				<table class="gTab">
					<tr>
						...
						<td style="width:400px;text-align:right">
							...
							<s:if test="#request.ProList[#st.index].create_user_id == #session.user_id">
	...
	<script type="text/javascript">
		...
		function EditPro(rowID){
			var id = $("#ProjectId_"+rowID).val();
			CreatePopDiv("Description","70%","70%","ProjectDescription?id="+id);
		}
		
		function Attach(rowID){
			var id = $("#ProjectId_"+rowID).val();
			CreatePopDiv("Download","70%","70%","ProjectDownload?id="+id);
		}
		
		function OpenSearch(){
			CreatePopDiv("Search","70%","70%","ProjectSearch");
		}
		...
		function ShowUsers(rowID){
			var id = $("#ProjectId_"+rowID).val();
			CreatePopDiv("ShowUsers","70%","70%","ProjectUsers?id="+id);
		}
		
		function OpenGraph(rowID){
			var id = $("#ProjectId_"+rowID).val();
			CreatePopDiv("OpenGraph","95%","85%","ProjectGraph?id="+id);
		}
		...
		function AppendList(objList){
			...
			if(objList.length>0){
				for(var i=0;i<objList.length;i++){
					...
					list_Str += "<a href='javascript:void(0);' onclick='OpenHistory("+len+");'><img src='<%=basePath%>image/inbox_full.png'><s:text name='history' /></a>";
					if(objList[i][4]>0){
		...
		function OpenHistory(rowID){
			var id = $("#ProjectId_"+rowID).val();
			CreatePopDiv("OpenHistory","70%","70%","ProjectHistory?id="+id);
		}
	</script>
</body>
</html>