...
<body style="width:100%;height:100%;">
			...
			<table class="gTab">
				<tr>
					...
					<td style="width:200px;float:right;">
						<table class="gTab">
							<tr>
								<td><a href="javascript:void(0);" onclick="javascript:document.getElementById('showall').checked = true;window.location.href='MsgList';"><s:text name="showall" /></a></td>
								<td><span class="toggle-bg"> <input type="radio" id="showall" name="toggle" value="off" onclick="javascript:window.location.href='MsgList'" /> <input type="radio" id="unread" name="toggle" value="on" onclick="javascript:window.location.href='MsgList?showwhat=1'" <s:if test="#request.MsgInfo.showwhat == 1">checked</s:if> /> <span class="switch" />
								</span></td>
								<td><a href="javascript:void(0);" onclick="javascript:document.getElementById('unread').checked = true;window.location.href='MsgList?showwhat=1';"><s:text name="unread" /></a></td>
							</tr>
						</table></td>
				</tr>
			</table>
		...
		<s:iterator value="#request.MsgList" status="st">
			<div style="border-bottom:1px dashed #E5E5E5">
				<table class="gListTab" <s:if test="#request.MsgList[#st.index].is_read==0">style="font-weight:bold;"</s:if>>
					<tr>
						<td style="width:20px;"><input type="hidden" id="MsgID_<s:property value="#st.index"/>" value="<s:property value="#request.MsgList[#st.index].id" />"/><input name="SelectMsg" type="checkbox" value="<s:property value="#st.index"/>"></td>
						<td style="width:100px;"><s:property value="#request.MsgList[#st.index].src_type_name" /></td>
						...
						<td style="width:60px;"><a href="javascript:void(0);" onclick="Delete(<s:property value="#st.index"/>);"><img src="<%=basePath%>image/close.png"><s:text name="delete" /></a></td>
					</tr>
					<tr>
	...
	<script type="text/javascript">
		...
		function Add(){
			CreatePopDiv("Add","70%","70%","<%=basePath%>msg_edit.jsp");
		}
		...
		function SearchMsg(){
			CreatePopDiv("Search","70%","40%","MsgSearch");
		}
		...
		function AppendList(objList){
			...
			if(objList.length>0){
				for(var i=0;i<objList.length;i++){
					...
					list_Str += "<td><a href='javascript:void(0);' onclick=''>"+objList[i][2]+"</a></td>";
